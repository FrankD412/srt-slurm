...
def _acpi_body(*, include_grace=True):
    lines = [
        ...
        'cpu_power_acpi_watts{sensor="a/0",type="cpu",socket="0",oem_info="CPU Power Socket 0"} 48.1\n',
        'cpu_power_acpi_watts{sensor="a/1",type="sysio",socket="0",oem_info="SysIO Power Socket 0"} 5.2\n',
    ]
    if include_grace:
        lines.append(
            'cpu_power_acpi_watts{sensor="a/2",type="grace",socket="0",oem_info="Grace Power Socket 0"} 93.4\n'
        )
...
def test_acpi_mode_totals_only_grace_channels():
    scrape = parse_cpu_scrape(_acpi_body())

    assert scrape.mode == "acpi"
    kinds = {r.kind for r in scrape.readings}
    assert kinds == {"cpu", "sysio", "grace"}
    assert scrape.total_power_w == 93.4


def test_acpi_mode_leaves_total_blank_without_a_grace_channel():
