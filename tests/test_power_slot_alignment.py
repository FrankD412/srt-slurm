...
class TestClassification:
    ...
    def test_truncated_ranges_attribute_later_gaps_to_truncation_not_lost_accounting(self):
        """When the producer capped the range list, no-row slots after the last retained range are
        `missed:truncated`, and only gaps *before* that point can still be `unaccounted`."""
        rows = [_row("a", s, START + s + 0.1) for s in range(10)]
        rows += [_row("b", s, START + s + 0.1) for s in (0, 2, 4, 6, 8)]  # b misses every odd slot
        m = _manifest(scrape_count=10, missed=[("b", 1, 1, Reason.SAMPLE_SCHEDULE_OVERRUN)])
        m["missed_sample_count"] = 5  # exact, as the producer keeps it
        m["missed_sample_ranges_truncated"] = True  # but only the first range survived
        al = compute_slot_alignment(rows, m)
        assert al is not None
        b = {s: al.cells[s]["b"] for s in range(10)}
        assert b[1].label == "missed:overrun"
        assert all(b[s].label == "missed:truncated" for s in (3, 5, 7, 9))
        host_b = next(h for h in al.per_host if h.hostname == "b")
        assert host_b.unaccounted == 0
        assert host_b.missed == {"overrun": 1, "truncated": 4}

        # Without the flag the same manifest is genuinely inconsistent and says so.
        m["missed_sample_ranges_truncated"] = False
        al2 = compute_slot_alignment(rows, m)
        assert al2 is not None
        assert next(h for h in al2.per_host if h.hostname == "b").unaccounted == 4
